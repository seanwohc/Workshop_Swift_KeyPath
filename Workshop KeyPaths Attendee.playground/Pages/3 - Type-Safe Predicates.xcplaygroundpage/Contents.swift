...
// It's also stringly typed and throws runtime error 😭
...
struct Predicate<Element> {
    ...
    init(condition: @escaping (Element) -> Bool) {
        self.evaluator = condition
    }
...
func > <Element, Value: Comparable>(_ keyPath: KeyPath<Element, Value>, _ constant: Value) -> Predicate<Element> {
    // 👩🏽‍💻👨🏼‍💻 Implement `func >`
    return Predicate(condition: { (element:Element) -> Bool in
        element [keyPath: keyPath] > constant
    })
}
...
func && <Element>(_ lhs: Predicate<Element>, _ rhs: Predicate<Element>) -> Predicate<Element> {
    // 👩🏽‍💻👨🏼‍💻 Implement `func &&`
    return Predicate { (element: Element) -> Bool in
        lhs.evaluate(with: element) && rhs.evaluate(with: element)
    }
